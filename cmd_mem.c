...
int do_mem_md ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	//printf("david md: addr= %x, size = %d, length = %x\n", addr, size, length);
	/* Print the lines.
	 *
...
int do_mem_mck ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	ulong	addr_0, addr_1, length;
	ulong	i, nbytes, linebytes;
	int	size;
	int is256MB = 0;
	 ...
	 * entered.
	 */
	addr_0 = 0x00000000; // 0MB
	addr_1 = 0x10000000; // 256MB
	size = 4;
	length = 0x40;
#if 0
	if (argc < 2) {
...
#endif
	//printf("md1: addr= %x, size = %d, length = %x\n", addr, size, length);
#if 0
	if ((flag & CMD_FLAG_REPEAT) == 0) {
		/* New command specified.  Check for a size specification.
		 * Defaults to long if no or incorrect specification.
	...
	do {
		char	linebuf[DISP_LINE_LEN];
		uint	*uip_0 = (uint   *)linebuf;
		uint    *uip_1 = (uint   *)linebuf;	
		
		//printf("addr_0 %08lx:  addr_1 %08lx:", addr_0, addr_1);
		linebytes = (nbytes>DISP_LINE_LEN)?DISP_LINE_LEN:nbytes;

		for (i=0; i<linebytes; i+= size) {
			if (size == 4) {
				//printf(" %08x", (*uip++ = *((uint *)addr)));
				if(((*uip_0 = *((uint *)addr_0))== (*uip_1 = *((uint *)addr_1))))
				{
					is256MB = 1;
					/*
					printf("\n *uip_0 = %08x %s *uip_1 = %08x\n", *uip_0,  (is256MB=1?"==":"!="
), *uip_1);
					*/
				}
				else
